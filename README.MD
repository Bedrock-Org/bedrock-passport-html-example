...
`Authorization: Bearer <accessToken>`, then call `signOut()` to clear local auth state. Store `token`/`refreshToken` on the callback (see Step&nbsp;4) so the access token is available for the logout call.
